...
clsConfirmDlg2 = class("clsConfirmDlg2", clsBaseUI)

function clsConfirmDlg2:ctor(parent)
	clsBaseUI.ctor(self, parent, "uistu/ConfirmDlg2.csb")
...
function clsConfirmDlg2:dtor()
	--g_EventMgr:FireEvent("hiddenNotice")
end

function clsConfirmDlg2:Reflesh(sTitle, sTips, callback, sOk, sNo, iStyle)
...
--	if sTips then self.lblTips:setString(sTips) end
	self.lblTips:setPosition(206,145)
	if iStyle then
	...
	if sTips then 
		...
		for i, str in ipairs(strTbl) do
			...
			lbl:setPosition(self.BkgFrame:getContentSize().width*0.5, self.BkgFrame:getContentSize().height*0.55+((#strTbl+1)/2-i)*42)
