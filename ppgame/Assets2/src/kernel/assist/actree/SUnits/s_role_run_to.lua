...
local s_role_run_to = class("s_role_run_to", clsPromise)

s_role_run_to._default_args = default_args

function s_role_run_to:ctor(argInfo, Ctx)
	clsPromise.ctor(self, argInfo, Ctx)
	self._node_type = "s_role_run_to"
end

function s_role_run_to:OnStop()
...
function s_role_run_to:OnProc()
	...
	tmpChar:DoRun(dx, dy, distance, function()
		self:Done()
	end)
end

return s_role_run_to
