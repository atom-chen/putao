...
key = "LHPxyou520"
sign = "xxtea"
outDir = "pack_android/release/res"
assetsDir = {
...
def encrypt_res():
    ...
    cmd = subprocess.Popen('%s/quick/bin/encrypt_res.bat -i res -o %s -ek %s -es %s' \
...
def compile_laucher():
    ...
    cmd = subprocess.Popen('%s/quick/bin/compile_scripts.bat -i src -o %s/laucher.zip -x app,kernel,lib,cocos -e xxtea_chunk -ek %s -es %s -b 32' \
            %(engineRoot,outDir,key,sign), shell=True,env=new_env)
    cmd.wait()
    if cmd.returncode != 0:
        print "Error while building, check error above!"
...
def compile_cocos():
    ...
    cmd = subprocess.Popen('%s/quick/bin/compile_scripts.bat -i src -o %s/cocos.zip -x app,kernel,lib,laucher -e xxtea_chunk -ek %s -es %s -b 32' \
            %(engineRoot,outDir,key,sign), shell=True,env=new_env)
    cmd.wait()
    if cmd.returncode != 0:
        print "Error while building, check error above!"
...
def compile_lib():
    ...
    cmd = subprocess.Popen('%s/quick/bin/compile_scripts.bat -i src -o %s/lib.zip -x app,kernel,cocos,laucher -e xxtea_chunk -ek %s -es %s -b 32' \
            %(engineRoot,outDir,key,sign), shell=True, env=new_env)
    cmd.wait()
    if cmd.returncode != 0:
        print "Error while building, check error above!"
...
def compile_kernel():
    ...
    cmd = subprocess.Popen('%s/quick/bin/compile_scripts.bat -i src -o %s/kernel.zip -x app,lib,cocos,laucher -e xxtea_chunk -ek %s -es %s -b 32' \
            %(engineRoot,outDir,key,sign), shell=True, env=new_env)
    cmd.wait()
    if cmd.returncode != 0:
        print "Error while building, check error above!"
...
def compile_app():
    ...
    cmd = subprocess.Popen('%s/quick/bin/compile_scripts.bat -i src -o %s/app.zip -x kernel,lib,cocos,laucher -e xxtea_chunk -ek %s -es %s -b 32' \
            %(engineRoot,outDir,key,sign), shell=True, env=new_env)
    cmd.wait()
    if cmd.returncode != 0:
        print "Error while building, check error above!"
...
def GenerateprojectManifestPath():
    ...
    for f in fileList:
        f2 = {"md5" : CalcMD5(f)}
        #f1 = "res" + f[6:]
        f = f.replace('pack_android/release/res','res')
        print f
        dataDic[f] = f2
        #print f1
