...
function cutDebugInfo($contents){
    //第5个字节 存储debug信息中lua文件路径长度 + "@" 的长度  debug = “@” + path
    $debugInfoLenBytes = stringToBytes(substr($contents,5,1));
    $debugInfoLen = $debugInfoLenBytes[0];
    ...
    if($pos === false){
        ...
        //修改debug 信息长度
        $contentsPrefixBytes[5] = $debugInfoLen;
