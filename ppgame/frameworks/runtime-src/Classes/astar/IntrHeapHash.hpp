...
public:
  ...
  {
    hash=(Node**)malloc(sizeof(Node*)*preAlloc);
    memset(hash,0,preAlloc*sizeof(Node*));
    hashSize=preAlloc;
    heap=(Node**)malloc(sizeof(Node*)*preAlloc);
  ...
  T* Insert(T* value)
  {
    if((uint32)collisionsCount>hashSize/2)
    {
      Rehash();
    }
    ...
    {
      heapAlloc*=2;
      Node** newHeap=(Node**)malloc(sizeof(Node)*heapAlloc);
...
protected:
  ...
  void Rehash()
  {
    ...
    hashSize*=2;
    hash=(Node**)malloc(sizeof(Node*)*hashSize);
    memset(hash,0,hashSize*sizeof(Node*));
    collisionsCount=0;
