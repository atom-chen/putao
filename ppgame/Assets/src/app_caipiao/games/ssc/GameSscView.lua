...
clsGameSscView = class("clsGameSscView", clsBaseUI)

function clsGameSscView:ctor(parent,gameArg)
	clsBaseUI.ctor(self, parent, "uistu/GameSSC.csb")
	...
	self._gameObj = ClsGameSscMgr.GetInstance()
...
function clsGameSscView:dtor()
	ClsLayerManager.GetInstance():SetKeyboardAniDelegate()
	ClsGameSscMgr.GetInstance():DelListener(self)
	ClsGameMgr.GetInstance():LeaveGame()
end

function clsGameSscView:SetGameArg(gameArg)
	self._kithe = nil 
...
--注册控件事件
function clsGameSscView:InitUiEvents()
	...
	--		self._billPaper:SendBill(self._gameObj)
	--	end
		self._billPaper:SetCost(self:GetCost())
		if self._billPaper:CheckValid(self._gameObj,true,true) then
...
function clsGameSscView:quitListener()
...
function clsGameSscView:RandSelect()
...
-- 注册全局事件
function clsGameSscView:InitGlbEvents()
...
function clsGameSscView:CreateAnimation()
...
function clsGameSscView:RefleshWangqi()
...
function clsGameSscView:on_req_bet_open_result(recvdata,tArgs)
...
function clsGameSscView:on_req_bet_game_state(recvdata,tArgs,bFromCache)
...
function clsGameSscView:on_req_goucai_game_wanfa_list(recvdata,tArgs)
...
function clsGameSscView:OnCurWanFa(info)
	print("选择游戏玩法", info.name)
	logger.dump(info)
...
function clsGameSscView:on_req_goucai_game_qiuhao_peilv_list(recvdata,tArgs)
	local data = recvdata and recvdata.data
	if not data then return end
	if not gameutil.IsSameGid(tArgs.gid, self.gameArg.gid) then return end
...
function clsGameSscView:RefleshBallsArea(data)
	...
	for iii, group in ipairs(self._selectedWanFa.play) do
		local BtnFeature = self:CreateTitleBtn(120,50,group.name)
		self._titleBtns[iii] = BtnFeature
		autoGrid:AddRow({BtnFeature}, 1, 120, 50, "center", 0)
		KE_SetParent(BtnFeature, self.ScrollViewGame)
		autoGrid:AddSpace(10)
		local objList = {}
		for _, ball in ipairs(data.balls_rate[group.id].balls) do
			local BtnFeature = self:CreateFeatureBtn(160,105,ball.name, self._gameObj:CalculatePeiLv(ball))
		...
		autoGrid:AddRow(objList, 4, 160,105, "left", 10)
		if iii ~= #self._selectedWanFa.play then
			autoGrid:AddSpace(20)
		end
	end
	autoGrid:AddSpace(5)
...
function clsGameSscView:CreateTitleBtn(btnWid, btnHei, name)
	return UIFactory.CreateMenuBtn(btnWid, btnHei, name)
end

function clsGameSscView:CreateFeatureBtn(wid,hei,name,rate)
	return UIFactory.CreateRectangleButton(parent, wid, hei, name, rate)
end

function clsGameSscView:OnSelectBallChanged()
	local strBalls = ""
	for _, BtnFeature in pairs(self._btnFeatureList) do
	...
	--
	if not self._selectedWanFa or not self._curData or not self._btnFeatureList or not self._titleBtns then return end
	local data = self._curData
	...
	if betCount > 0 then
		...
		--self.ScrollViewGame:setContentSize(cc.size(preSize.width,self:GetAdaptInfo().hAuto))
	else
	...
	autoGrid:AddSpace(5)
	for iii, group in ipairs(self._selectedWanFa.play) do
		autoGrid:AddRow({self._titleBtns[iii]}, 1, 120, 50, "center", 0)
		autoGrid:AddSpace(10)
		local objList = {}
		for _, ball in ipairs(data.balls_rate[group.id].balls) do
			idx = idx + 1
...
function clsGameSscView:ClearSelectedBalls()
	for _, BtnFeature in pairs(self._btnFeatureList) do
		BtnFeature:SetSelectedQuiet(false)
	end
	self._billPaper:Clear()
...
function clsGameSscView:GetCost()
	local money = self.EditMoney:getString() or ""
	local pos = string.find(money, "%.")
	if pos then
		money = string.sub(money, 1, pos+1)
