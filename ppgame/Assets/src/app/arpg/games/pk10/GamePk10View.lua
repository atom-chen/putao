...
clsGamePk10View = class("clsGamePk10View", clsBaseUI)

function clsGamePk10View:ctor(parent,gameArg)
	clsBaseUI.ctor(self, parent, "uistu/GamePk10.csb")
	...
	self._gameObj = ClsGamePk10Mgr.GetInstance()
...
function clsGamePk10View:dtor()
	ClsLayerManager.GetInstance():SetKeyboardAniDelegate()
	ClsGamePk10Mgr.GetInstance():DelListener(self)
	ClsGameMgr.GetInstance():LeaveGame()
end

function clsGamePk10View:SetGameArg(gameArg)
	self._kithe = nil 
...
--注册控件事件
function clsGamePk10View:InitUiEvents()
...
function clsGamePk10View:quitListener()
...
function clsGamePk10View:RandSelect()
...
-- 注册全局事件
function clsGamePk10View:InitGlbEvents()
...
function clsGamePk10View:CreateAnimation()
		...
		spr:runAction(cc.RepeatForever:create(
			cc.Sequence:create(
				...
				cc.CallFunc:create(function()
					...
					spr:setString(string.format("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni,spr._idxAni))
				end)
			)
		))
	end
...
function clsGamePk10View:RefleshWangqi()
...
function clsGamePk10View:on_req_bet_open_result(recvdata,tArgs)
...
function clsGamePk10View:on_req_bet_game_state(recvdata,tArgs,bFromCache)
...
function clsGamePk10View:on_req_goucai_game_wanfa_list(recvdata,tArgs)
...
function clsGamePk10View:OnCurWanFa(info)
	print("选择游戏玩法")
	logger.dump(info)
...
function clsGamePk10View:on_req_goucai_game_qiuhao_peilv_list(recvdata,tArgs)
	local data = recvdata and recvdata.data
	if not data then return end
	if not gameutil.IsSameGid(tArgs.gid, self.gameArg.gid) then return end
...
function clsGamePk10View:RefleshBallsArea(data)
...
function clsGamePk10View:CreateTitleBtn(btnWid, btnHei, name)
	return UIFactory.CreateMenuBtn(btnWid, btnHei, name)
end

function clsGamePk10View:CreateFeatureBtn(wid,hei,name,rate)
	return UIFactory.CreateRectangleButton(parent, wid, hei, name, rate)
end

function clsGamePk10View:OnSelectBallChanged()
	for _, BtnFeature in pairs(self._btnFeatureList) do
...
function clsGamePk10View:ClearSelectedBalls()
	for _, BtnFeature in pairs(self._btnFeatureList) do
		BtnFeature:SetSelectedQuiet(false)
	end
	self._billPaper:Clear()
...
function clsGamePk10View:GetCost()
	local money = self.EditMoney:getString() or ""
	local pos = string.find(money, "%.")
	if pos then
		money = string.sub(money, 1, pos+1)
