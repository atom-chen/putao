...
clsGameYbView = class("clsGameYbView", clsBaseUI)

function clsGameYbView:ctor(parent,gameArg)
	clsBaseUI.ctor(self, parent, "uistu/GameYb.csb")
	...
	self._gameObj = ClsGameYbMgr.GetInstance()
...
function clsGameYbView:dtor()
	ClsLayerManager.GetInstance():SetKeyboardAniDelegate()
	ClsGameYbMgr.GetInstance():DelListener(self)
	ClsGameMgr.GetInstance():LeaveGame()
end

function clsGameYbView:SetGameArg(gameArg)
	self._kithe = nil 
...
--注册控件事件
function clsGameYbView:InitUiEvents()
...
function clsGameYbView:quitListener()
...
function clsGameYbView:RandSelect()
...
-- 注册全局事件
function clsGameYbView:InitGlbEvents()
...
function clsGameYbView:CreateAnimation()
...
function clsGameYbView:RefleshWangqi()
...
function clsGameYbView:on_req_bet_open_result(recvdata,tArgs)
...
function clsGameYbView:on_req_bet_game_state(recvdata,tArgs,bFromCache)
...
function clsGameYbView:on_req_goucai_game_wanfa_list(recvdata,tArgs)
...
function clsGameYbView:OnCurWanFa(info)
	print("选择游戏玩法")
	logger.dump(info)
...
function clsGameYbView:on_req_goucai_game_qiuhao_peilv_list(recvdata,tArgs)
	local data = recvdata and recvdata.data
	if not data then return end
	if not gameutil.IsSameGid(tArgs.gid, self.gameArg.gid) then return end
...
function clsGameYbView:RefleshBallsArea(data)
	...
	for iii, group in ipairs(self._selectedWanFa.play) do
		...
		for _, ball in ipairs(data.balls_rate[group.id].balls) do
			local BtnFeature = self:CreateFeatureBtn(156,105,ball.name, self._gameObj:GetBallRateInfo(ball).rate)
		...
		autoGrid:AddRow(objList, 4, 156,105, "left", 10)
...
function clsGameYbView:CreateTitleBtn(btnWid, btnHei, name)
	return UIFactory.CreateMenuBtn(btnWid, btnHei, name)
end

function clsGameYbView:CreateFeatureBtn(wid,hei,name,rate)
	return UIFactory.CreateRectangleButton(parent, wid, hei, name, rate)
end

function clsGameYbView:OnSelectBallChanged()
	for _, BtnFeature in pairs(self._btnFeatureList) do
...
function clsGameYbView:ClearSelectedBalls()
	for _, BtnFeature in pairs(self._btnFeatureList) do
		BtnFeature:SetSelectedQuiet(false)
	end
	self._billPaper:Clear()
...
function clsGameYbView:GetCost()
	local money = self.EditMoney:getString() or ""
	local pos = string.find(money, "%.")
	if pos then
		money = string.sub(money, 1, pos+1)
